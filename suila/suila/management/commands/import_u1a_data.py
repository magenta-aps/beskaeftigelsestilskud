    ...
    def _import_data(
        ...
        for person in persons:
            ...
            for item in person_akap_u1a_items:
                if item.u1a.id not in u1a_items_dict:
                    u1a_items_dict[item.u1a.id] = []
            ...
            for _, u1a_items in u1a_items_dict.items():
                ...
                # Get U1A Employer
                try:
                    u1a_employer = Employer.objects.filter(cvr=u1a.cvr).first()
                except Employer.DoesNotExist:
                    u1a_employer = Employer.objects.create(
                        cvr=u1a.cvr,
                        name=u1a.virksomhedsnavn,
                        load=data_load,
                    )
                    self._write_verbose("\t- Created 1 Employer object")

                # Get U1A PersonMonth, or create it
                try:
                    u1a_person_month = PersonMonth.objects.filter(
                        person_year=person_year, month=u1a_month
                    ).first()
                except PersonMonth.DoesNotExist:
                    u1a_person_month = PersonMonth.objects.create(
                ...
                if not u1a_person_month:
                    logger.warning(
                        (
                            "Unable to find, or create, PersonMonth object for: "
                            f"PersonYear={person_year}, Month={u1a_month}"
                        )
                    )
                    continue
