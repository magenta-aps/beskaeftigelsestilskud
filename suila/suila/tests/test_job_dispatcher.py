...
class TestJobDispatcherCommands(TestCase):
    ...
    def test_job_dispatch_calls_second_tuesday(
        self,
        mock_call_command: MagicMock,
    ):
        # JobDispatcher date which is the second "tuesday" in the month
        job_dispatcher_test_date = datetime(2025, 4, 8)

        # Mocking
        def _mock_call_command_logic(command_name, *args, **options):
            # Handle command "_param"'s
            cmd_param_year = (
                options.get("year", None)
                if command_name not in ["load_eskat"]
                else args[0]
            )
        ...
        mock_call_command.side_effect = _mock_call_command_logic

        # Invoke the 'job_dispatcher' with the specified date
        call_command(
            self.command,
