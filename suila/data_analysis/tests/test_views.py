...
class TestPersonYearEstimationEngineUpdateView(
    TestViewMixin, PersonYearEstimationSetupMixin, TestCase
):
    ...
    def setUpTestData(cls):
        super().setUpTestData()
        cls.url_a = reverse(
            "data_analysis:set_personyear_estimation_engine_a",
            kwargs={"pk": cls.person_year.pk},
        )
        cls.url_u = reverse(
            "data_analysis:set_personyear_estimation_engine_u",
            kwargs={"pk": cls.person_year.pk},
        )
        cls.data_a = {"preferred_estimation_engine_a": "MonthlyContinuationEngine"}
        cls.data_u = {"preferred_estimation_engine_u": "MonthlyContinuationEngine"}

    def test_edit_person_as_admin(self):

        # Check that engines equal their defaults
        self.assertEqual(
        ...
        self.client.post(self.url_a, self.data_a)
        ...
        # Change engine_u
        self.client.post(self.url_u, self.data_u)
    ...
    def test_history(self):
        ...
        self.client.post(self.url_a, data=self.data_a)
        self.person_year.refresh_from_db()

        latest_history_entry = self.person_year.history.order_by("-history_date")[0]

        self.assertEqual(
            latest_history_entry.preferred_estimation_engine_a,
            "MonthlyContinuationEngine",
        )
