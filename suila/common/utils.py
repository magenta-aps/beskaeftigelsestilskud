...
def get_people_in_quarantine(year: int, cpr_numbers: Iterable[str]) -> pd.DataFrame:
    ...
    df = to_dataframe(
        ...
        dtypes={
            ...
            "prior_benefit_transferred": float,
            "benefit_transferred": float,
        },
    )
    ...
    df.fillna({"quarantine_reason": QuarantineReason.NONE}, inplace=True)
    df.fillna({"in_quarantine": False}, inplace=True)
    return df
